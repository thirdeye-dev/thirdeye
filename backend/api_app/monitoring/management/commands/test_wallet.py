...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--send', action='store_true', help='Send Ether to another address')
    ...
    def handle(self, *args, **options):
        ...
        if not private_key or not public_address:
            self.stdout.write(self.style.ERROR('Missing ETH_PRIVATE_KEY or ETH_PUBLIC_ADDRESS environment variable'))
            return
    ...
    def send_ether(self, account, w3):
        ...
        self.stdout.write(self.style.SUCCESS(f'Transaction sent: {transaction_hash.hex()}'))
