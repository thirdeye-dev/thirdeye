...
class ContractAddressSerializer(serializers.Serializer):
    ...
    contract_addresses = serializers.ListField(child=serializers.CharField(min_length=42, max_length=42))
...
class BlockchainAlertRunner:
    ...
    def check_alert_condition(self, alert):
        attribute_key = alert['attribute'].replace('{{ $transaction.', '').replace(' }}', '')
